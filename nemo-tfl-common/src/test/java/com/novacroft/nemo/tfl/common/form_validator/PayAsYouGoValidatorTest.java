...
import static com.novacroft.nemo.test_support.CardInfoResponseV2TestUtil.*;
...
import static com.novacroft.nemo.test_support.CardTestUtil.LOW_CREDIT_BALANCE;
...
/**
 * Pay as you go validator unit tests
 */
public class PayAsYouGoValidatorTest {
    private PayAsYouGoValidator validator;
    ...
    private static final Integer CREDIT_BALANCE = 1000;

    @Before
    public void setUp() {
        validator = new PayAsYouGoValidator();
    ...
    public void shouldValidate() {
        cmd.setCreditBalance(CREDIT_BALANCE);
    ...
    @Test
    public void shouldValidateWithCreditBalance() {
        cmd.setCreditBalance(CREDIT_BALANCE);
    ...
    @Test
    public void shouldvalidatePendingItems() {
        cmd.setCreditBalance(CREDIT_BALANCE);
    ...
    @Test
    public void shouldValidateWithPayAsYouGoBalanceIsLessThanLimitInSystem() {
        when(mockCardDataService.findById(anyLong())).thenReturn(getTestCardDTO1());
    ...
    @Test
    public void shouldNotValidateWithLowCreditBalance() {
        cmd.setCreditBalance(LOW_CREDIT_BALANCE);
        cmd.setAutoTopUpAmt(AUTO_TOPUP_AMT);
        Errors errors = new BeanPropertyBindingResult(cmd, "cmd");
        validator.validate(cmd, errors);
    ...
    @Test
    public void shouldNotValidateWithNullCreditBalance() {
    ...
    public void shouldNotValidateWithPayAsYouGoBalanceIsGreaterThanLimitInSystem() {
        when(mockCardDataService.findById(anyLong())).thenReturn(getTestCardDTO1());
        when(mockGetCardService.getCard(anyString())).thenReturn(getTestCardInfoResponseV2DTO2());
        when(mockSystemParameterService.getIntegerParameterValue(PAY_AS_YOU_GO_AD_HOC_PURCHASE_LIMIT.code())).thenReturn(CartItemTestUtil.PAY_AS_YOU_GO_AD_HOC_PURCHASE_LIMIT);
        when(mockSystemParameterService.getIntegerParameterValue(PAY_AS_YOU_GO_LIMIT.code())).thenReturn(CartItemTestUtil.PAY_AS_YOU_GO_LIMIT);
    ...
    @Test
    public void shouldNotValidateIfPayAsYougoCreditIsGreaterThanPurchaseLimit(){
        when(mockSystemParameterService.getIntegerParameterValue(PAY_AS_YOU_GO_AD_HOC_PURCHASE_LIMIT.code())).thenReturn(CartItemTestUtil.PAY_AS_YOU_GO_AD_HOC_PURCHASE_LIMIT);
        
        cmd.setCreditBalance(HIGH_CREDIT_BALANCE+500);
        cmd.setAutoTopUpAmt(AUTO_TOPUP_AMT);
        cmd.setCardId(CARD_ID);
        Errors errors = new BeanPropertyBindingResult(cmd, "cmd");
        validator.validate(cmd, errors);
