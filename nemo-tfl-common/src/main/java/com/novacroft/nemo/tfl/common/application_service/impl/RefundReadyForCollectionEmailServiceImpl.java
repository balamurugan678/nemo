...
/**
 * Email user when refund is ready for collection implementation
 */
@Service(value = "refundReadyForCollectionEmailService")
public class RefundReadyForCollectionEmailServiceImpl extends BaseRefundEmailService implements EmailMessageService {
    protected static final Logger logger = LoggerFactory.getLogger(RefundReadyForCollectionEmailServiceImpl.class);

    @Override
    protected String getSubject(EmailArgumentsDTO arguments) {
        return getContent(ContentCode.REFUND_READY_FOR_COLLECTION_EMAIL_SUBJECT.textCode());
    }

    @Override
    protected String getBody(EmailArgumentsDTO arguments) {
        return getContent(ContentCode.REFUND_READY_FOR_COLLECTION_EMAIL_BODY.textCode(),
