 ...
 */
@Service(value = "refundPickUpWindowExpiredEmailService")
public class RefundPickUpWindowExpiredEmailServiceImpl extends BaseRefundEmailService implements EmailMessageService {
    protected static final Logger logger = LoggerFactory.getLogger(RefundPickUpWindowExpiredEmailServiceImpl.class);

    @Override
    protected String getSubject(EmailArgumentsDTO arguments) {
        return getContent(ContentCode.REFUND_PICK_UP_WINDOW_EXPIRED_EMAIL_SUBJECT.textCode());
    }

    @Override
    protected String getBody(EmailArgumentsDTO arguments) {
        return getContent(ContentCode.REFUND_PICK_UP_WINDOW_EXPIRED_EMAIL_BODY.textCode(),
