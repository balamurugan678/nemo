...
var s = s_gi(tfl.stats.account);

s.pageName = tfl.stats.pageName;
s.server = tfl.stats.server;
s.channel = tfl.stats.channel;
s.pageType = tfl.stats.pageType;
s.prop1 = tfl.stats.prop1;
s.prop2 = tfl.stats.prop2;
s.prop3 = tfl.stats.prop3;
s.prop4 = tfl.stats.prop4;
s.prop5 = tfl.stats.prop5;
s.prop11 = tfl.stats.prop11;
s.prop12 = tfl.stats.prop12;
s.campaign = tfl.stats.campaign;
s.state = tfl.stats.state;
s.zip = tfl.stats.zip;
s.products = tfl.stats.products;
s.purchaseID = tfl.stats.purchaseID;
s.eVar1 = tfl.stats.eVar1;
s.eVar2 = tfl.stats.eVar2;
s.eVar3 = tfl.stats.eVar3;
s.eVar4 = tfl.stats.eVar4;
s.eVar5 = tfl.stats.eVar5;
...
s.linkInternalFilters = "javascript:,.tfl.gov.uk";
...
function s_doPlugins(s) {
    ...
    /* Get CID from URL for online campaigns */
    s.campaign = s.getValOnce(s.getQueryParam('cid'), "extcamp", 0);
    /*Note: Please change 'CID' to whatever the campaign query param is for the campaigns "?CID=3567" */

    /* Internal Campaign Tracking */
    if (!s.eVar1) s.eVar1 = s.getQueryParam('intcmp');//Set internal campaign here if not set in page already.
    ...
    s.prop32 = s.getDaysSinceLastVisit();
    s.eVar8 = s.prop32;
    ...
    s.prop6 = s.eVar4 = s.getTimeParting('h', '0'); // Set hour
    s.prop7 = s.eVar5 = s.getTimeParting('d', '0'); // Set day
    s.prop8 = s.eVar6 = s.getTimeParting('w', '0'); // Set weekday

    //JS version
    s.prop23 = "H.25.2";
    // Page URL excluding Query Strings
    s.prop22 = location.protocol + "//" + location.host + location.pathname;

    /* Logged In Status Pathing Variables - prop13 Must be set on all pages */
    if (s.pageName && s.prop13 == 'Logged In') s.prop14 = s.prop13 + ":" + s.pageName;
    if (s.pageName && s.prop13 == 'Logged Out') s.prop14 = s.prop13 + ":" + s.pageName;

    // copy s.props to eVars
    //internal search
    if (s.prop11 && !s.eVar2) s.eVar2 = "D=c11";
    //internal search # of results
    if (s.prop12 && !s.eVar3) s.eVar3 = "D=c12";
    //Site sections
    if (s.prop1 && !s.eVar9) s.eVar9 = "D=c1";
    if (s.prop2 && !s.eVar10) s.eVar10 = "D=c2";
    if (s.prop3 && !s.eVar11) s.eVar11 = "D=c3";
    if (s.prop4 && !s.eVar12) s.eVar12 = "D=c4";
    if (s.prop5 && !s.eVar13) s.eVar13 = "D=c5";
    //custom pageName
    s.eVar20 = s.pageName;

    s.detectRIA('s_ria', 'prop33', 'prop34', '', '', '');
    ...
    s.Media.trackVars = "events,prop19,eVar32";
    s.Media.trackEvents = "event9,event10,event11";
...
adytt.p = function () {
    ...
    this.i = function (a) {
        ...
        if (!b) {
            this.p = document.getElementById(a);
            if (this.p) {
                if (a.match(/^[a-z0-9]+$/i)) {
                    window['adyttsc' + a] = function (o) {
                        adytt.pl[a].st(o)
                    };
                    this.p.addEventListener("onStateChange", "adyttsc" + a)
                } else {
                    this.p.addEventListener
                    ("onStateChange", "adytt.pl['" + a + "'].st")
                }
                var c = this;
                l(window, "unload", function (e) {
                    if (c.op) {
                        if (typeof c.sc === 'function') {
                            c.sc(c.po, true)
                        }
                    }
                })
    ...
    this.sc = function (a, b) {
        if (this.pl) {
            s.Media.stop(this.id, a);
            this.pl = false;
            this.tp = false
        }
        if (b && this.op) {
            s.Media.close(this.id);
            this.op = false;
    ...
    this.st = function (a) {
        ...
        switch (a) {
            case 0:
                this.sc(this.du, true);
                break;
            case 1:
                this.du = Math.floor(this.p.getDuration());
                if (!this.op) {
                    var b = this.p.getVideoUrl();
                    this.id = "YT|" + k(b, 'v');
                    if (typeof adytt.getID === 'function') {
                        this.id = adytt.getID({ url: b })
                    }
                ...
                break;
            case 2:
                this.sc(Math.floor(this.po), false);
                break;
            case 3:
                this.bl = this.p.getVideoBytesLoaded();
                this.po = this.p.getCurrentTime();
                if (this.bl === -1) {
                    this.sc(Math.floor(this.po), true)
                } else {
                    this.sc(Math.floor(this.po), false)
                }
                break;
    ...
    this.ti = function () {
        if (!document.getElementById(this.p.id)) {
            this.sc(this.po, true)
        } else if (this.p && (this.p.getPlayerState() === 1 || this.tp)) {
            this.po = this.p.getCurrentTime();
            this.bl = this.p.getVideoBytesLoaded();
            if (this.tp && this.bl > 0 && !this.pl) {
                this.pl = true;
                this.tp = false;
...
function onYouTubePlayerReady(a) {
    ...
    if (adytt.pl[a]) {
        adytt.pl[a].sc(Math.floor(adytt.pl[a].po), true);
        delete adytt.pl[a]
    }
    adytt.pl[a] = new adytt.p();
    adytt.pl[a].i(a)
}
...
s.visitorNamespace = "tfl";
s.trackingServer = "tfl.d3.sc.omtrdc.net";
