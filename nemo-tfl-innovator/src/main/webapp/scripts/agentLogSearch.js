var resultsTable;
    ...
    $(document).ready(function () {
        setElements();
        setEvents();
        onEvenSearchInvokeForCancelFunctionality(searchCriteriaNonEmptyFlag);
    ...
    function onEvenSearchInvokeForCancelFunctionality(searchCriteriaNonEmptyFlag) {
    	if(searchCriteriaNonEmptyFlag=="true") {
    		search();
    	}
    }
    ...
    function checkEmptySearch() {
        ...
        if((elementIdEndedAtVal.length==0) && (elementIdStartedAtVal.length==0)) {
        	empty = false;
        	alert("Please enter Job Extraction Dates \n" +
		        			" \t Start Date \n" +
		        			" \t End Date  ");
        	$("input[type='text']").first().focus();
        	return empty;
        } 
        else if(elementIdStartedAtVal.length==0) {
        	alert("Please enter Job Start Date");
        	empty = false;
        	$("#"+elementIdStartedAtVar).focus();
        	return empty;
        }
        else if(elementIdEndedAtVal.length==0){
        	alert("Please enter Job End Date");
        	empty = false;
        	$("#"+elementIdEndedAtVal).focus();
        	return empty;
        }
        else {
        	return empty;
    ...
    function clearCriteria() {
        ...
        $("input[type=text]").each(function () {
        	$(this).val("");
        	var elementIdJobName = pageName + "\\." +"jobName";  
        	$("#"+elementIdJobName).val("All");
        });
    }
