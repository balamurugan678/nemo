...
function toggleBackdateOtherReason() {
	
	 var backdatedOtherReasonId = pageName + "\\.cartItemCmd\\.backdatedOtherReason";
	 var backdatedOtherReasonField = 'label[for=' + backdatedOtherReasonId + '], input#' + backdatedOtherReasonId + '';
	 var BACKDATED_OTHERREASON_VALUE = 2;
	 
	 if ($("#backdatedReasonList").val() == BACKDATED_OTHERREASON_VALUE) {
		 	$(backdatedOtherReasonField).show();
		} else {
			$(backdatedOtherReasonField).hide();
		}
	    
	$("#backdatedReasonList").each(function(){
    	$(this).change(function(){
    		if ($(this).val() == BACKDATED_OTHERREASON_VALUE){
    			$(backdatedOtherReasonField).show();
    		} else {
    			$(backdatedOtherReasonField).hide();
    		}
...
function updateRefundCalculationBasisIfDeceasedCustomerSelected() {
	
	if( $(DECEASED_CUSTOMER_ID).is(":checked") ){
		$(".refundCalculationBasis").val('Pro Rata');
     } else {
    	 $(".refundCalculationBasis").val('Ordinary'); 
     }
}