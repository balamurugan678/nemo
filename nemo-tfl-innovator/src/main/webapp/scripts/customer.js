				...
				function() {
					...
					$("#openwebaccount-button")
							.click(
									function() {
										...
										var newLine = '\n';
										if (confirm(openWebAccountConfirmMessageCallLoggedPopupText
												+ newLine
												+ openWebAccountConfirmMessageLoginPopupText)) {
											$
													.ajax({
														type : "GET",
														url : "Customer/agentLogon.htm?webaccountId="
																+ customerId,
														success : function(
																response) {
														...
														error : function(e) {
															alert(getMessage(contentCode.OPEN_WEB_ACCOUNT_ERROR)
																	+ e);
														}
													});
										}
					...
					$("#orders-button").click(
							function() {
								...
								openLink(sAddress + "/Order.htm?customerId="
										+ customerId, "Orders:" + customerId,
										"Orders" + customerId + ".", "oyster",
										"oysteredit");
							});
...
function checkCustomer() {
	if ($("#" + pageName + "\\.firstName").val() != ''
			&& $("#" + pageName + "\\.lastName").val() != ''
			&& $("#" + pageName + "\\.postcode").val() != '') {
		$("#customerSelect").remove();
		$("#checkCustomerLink").remove();
		$.post(sAddress + '/Customer/checkCustomer.htm', $("form").serialize(),
				function(response) {
					...
					if (json.length > 0) {
						...
						$.each(json, function(i, opt) {
							opt.result = opt.firstName + " " + opt.lastName
									+ " -- [" + opt.address + "]";
						});
						$("#available").attr("class", "blank");
						var selectList = $("<select />").attr("id",
								"customerSelect");
						$("#customersFound").append(selectList);
						addOptionsValueAndText(json, $("#customerSelect"),
								"id", "result");
						var checkCustomerLink = $("<button />").attr("id",
								"checkCustomerLink").attr("value",
								sAddress + "/" + customerURL + "?id=").attr(
								"class", "customerLink").attr("type", "button")
								.html("Open").click(
										function() {
											var customerId = $(
...
function checkPrePayTicketFailed(e) {
	...
	$.post(sAddress + '/Customer/checkCardStatus.htm', {
		...
		if (confirm(obj.prePayTicketMessage)) {
			...
			if (parseInt(obj.prePayValueMessage, 10) > 0) {
				alert(getMessage(contentCode.PAY_AS_YOU_GO_BALANCE_ERROR)
						+ ": " + obj.prePayValueMessage + "\n\n"
...
function openCardAdmin(cardNumber, customerId) {
	openLink(sAddress + '/' + cardAdmin + "?cardNumber=" + cardNumber
			+ "&customerId=" + customerId, 'Card Admin', 'Card Admin',
			'oyster', 'oysteredit');
};

function checkCustomerIdForButton(buttonId) {
