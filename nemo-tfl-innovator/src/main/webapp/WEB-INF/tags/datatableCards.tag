...
<c:forEach items="${customerCards}" var="card" varStatus="status">
    ...
    <tr class="grid_row" id="row${status.count}">
        ...
        <td><to:anchor id="transferProduct" cssClass="button transferProduct" href="?id=${card.cardNumber}&cardId=${card.id}" secondaryId="transferProduct${status.count}"></to:anchor></td>
    </tr>
</c:forEach>
