...
public class AssignTaskToStageTwoApproval implements JavaDelegate {
    private static final Logger logger = LoggerFactory.getLogger(AssignTaskToStageTwoApproval.class);
    ...
    public void execute(DelegateExecution execution) throws Exception {
        logger.debug("In the AssignTaskToStageTwoApproval Class");
        logger.debug("execution id " + execution.getId());

        logger.debug("Assign the task to the StageTwoApproval group");
        ...
        try {
            ...
            taskService.addCandidateGroup(execution.getId(), AgentGroup.SECOND_STAGE_APPROVER.code());
        } catch (Exception e) {
