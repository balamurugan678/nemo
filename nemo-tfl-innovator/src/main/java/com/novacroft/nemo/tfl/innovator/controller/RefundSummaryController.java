...
@Controller
@RequestMapping(value = Page.INV_REFUND_SUMMARY)
@SessionAttributes(CART)
public class RefundSummaryController extends BaseController {
    static final Logger logger = LoggerFactory.getLogger(RefundSummaryController.class);
    ...
    public ModelAndView view(@ModelAttribute(PageCommand.CART) CartCmdImpl cmd) {
        return new ModelAndView(INV_REFUND_SUMMARY, CART, cmd);
    }

    @RequestMapping(params = TARGET_ACTION_BACK_TO_CUSTOMER_PAGE, method = RequestMethod.POST)
    public ModelAndView refund(@ModelAttribute(CART) CartCmdImpl cmd) {
